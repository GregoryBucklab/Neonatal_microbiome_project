...
# get 16s metadata
keep = metadata_all$SampleID %in% colnames(reads_table_16s)
metadata_16s = metadata_all[keep,]
metadata_16s_cha = metadata_all_cha[keep,]
metadata_16s_num = metadata_all_num[keep,]
...
metadata_16s$Group[metadata_16s$Group == 'control'] = 'TB'
metadata_16s$Group[metadata_16s$Group == 'preterm'] = 'PTB'
colnames(metadata_16s)[colnames(metadata_16s) == 'Group'] = 'Delivery'

metadata_16s_cha$Group[metadata_16s_cha$Group == 'control'] = 'TB'
metadata_16s_cha$Group[metadata_16s_cha$Group == 'preterm'] = 'PTB'
colnames(metadata_16s_cha)[colnames(metadata_16s_cha) == 'Group'] = 'Delivery'

rm(data,data_2,data_3)


# get 16s reads table and taxonomy
keep = rowSums(reads_table_16s) != 0
reads_table_16s = reads_table_16s[keep,]
...
data_2 = data_2[order(data_2$V1),]

keep = which(!duplicated(data$V2))
data$Taxa_name[keep] = data_2$V3
data$Taxonomy[keep] = data_2$V2
  
keep = which(duplicated(data$V2))
data$Taxa_name[keep] = data$Taxa_name[keep-1]
data$Taxonomy[keep] = data$Taxonomy[keep-1]

data$rowname_16s_2 = paste(data$Taxa_name,data$V3,sep = '_')
data$rowname_16s_2 = str_remove_all(data$rowname_16s_2,'_AT')
data$rowname_16s_2 = data$Taxa_name
taxonomy_data = data
rm(data,data_2)

taxa_list = unique(taxonomy_data$rowname_16s_2)

keep = !duplicated(taxonomy_data$rowname_16s_2)
taxonomy_data_2 = taxonomy_data[!keep,]
taxonomy_data = taxonomy_data[keep,]

duplicated_taxa_list = unique(taxonomy_data_2$rowname_16s_2)

for (a in 1: length(duplicated_taxa_list)) {
  n = which(taxonomy_data_2$rowname_16s_2 == duplicated_taxa_list[a])
  n= row.names(taxonomy_data_2)[n]
  n = as.numeric(n)
  m = which(taxonomy_data$rowname_16s_2 == duplicated_taxa_list[a])
  
  reads_table_16s[m,] = reads_table_16s[m,] + colSums(reads_table_16s[n,])
  reads_table_16s[n,] = rep(NA,ncol(reads_table_16s))
}

keep = !is.na(reads_table_16s[,1])
sum(keep)
reads_table_16s = reads_table_16s[keep,]
row.names(reads_table_16s) = taxonomy_data$rowname_16s_2
rm(taxonomy_data_2)

taxonomy_data = taxonomy_data[,c(2,5,6)]
colnames(taxonomy_data) = c('V1','V2','V3')

taxonomy = taxonomy_data
rm(taxonomy_data,taxonomy_data_2)

# remove clade
taxonomy$V2 = str_replace_all(taxonomy$V2, '_clade.*','')
taxonomy$V3 = str_replace_all(taxonomy$V3, '_clade.*','')

keep = duplicated(taxonomy$V3)
duplicated_taxa_list = unique(taxonomy$V3[keep])

for (a in 1: length(duplicated_taxa_list)) {
  n = which(taxonomy$V3 == duplicated_taxa_list[a])
  reads_table_16s[n[1],] = colSums(reads_table_16s[n,])
  reads_table_16s[n[2:length(n)],] = rep(NA,ncol(reads_table_16s))
  
  taxonomy$V3[n[2:length(n)]] = NA
}
...
keep = !is.na(taxonomy$V3)
sum(keep)
taxonomy = taxonomy[keep,]

row.names(reads_table_16s) = str_remove_all(row.names(reads_table_16s), '_clade.*')


####### pick up samples for MTS ######
metadata_ptb = metadata_PTB
metadata_ptb = metadata_ptb[metadata_ptb$SampleType == 'MV1D',]
metadata_ptb = metadata_ptb[order(metadata_ptb$VisitNum, decreasing = F),]

keep = duplicated(metadata_ptb$ParticipantID)
metadata_ptb = metadata_ptb[!keep,]

metadata_ptb = metadata_ptb $SampleID
setwd('/Users/binzhu/secure/fenn/vcu_gpfs2/home/bzhu/bccl/bzhu')
write.table(metadata_ptb,'ptb_MV1D_1st_visit.txt', quote = F, row.names = F, col.names = F)

setwd('/Users/binzhu/Desktop/Mom-baby')
metadata_ptb$weeks_pregnant
metadata_ptb$days_rel2birth
metadata_ptb = metadata_ptb[is.na(metadata_ptb$weeks_pregnant) | metadata_ptb$weeks_pregnant <= 21,]
...
setwd('/Users/binzhu/Desktop/mom_baby')
