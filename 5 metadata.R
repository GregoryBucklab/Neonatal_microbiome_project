...
# test association with composition of the microbiome
{
  ...
  output_3 = as.data.frame(matrix(data= NA, ncol = 5, nrow = ncol(module_sample_list)))
  colnames(output_3) = c('NB day 0','NB days 12','NR day 0','NR days 12','NS days 12')
  row.names(output_3) = paste0('Module_',c(0:(ncol(module_sample_list)-1)))

  output_4 = as.data.frame(matrix(data= NA, ncol = 5, nrow = ncol(module_sample_list)))
  colnames(output_4) = c('NB day 0','NB days 12','NR day 0','NR days 12','NS days 12')
  row.names(output_4) = paste0('Module_',c(0:(ncol(module_sample_list)-1)))
  ...
  # NB day 0
  {
    ...
    for (a in 1:(nrow(pvalue_all))) {
      ...
      d = vegdist(reads_table)
      perm.eg.betadisper <- betadisper(d, group = as.factor(metadata$`metadata[keep]`), type = "centroid")
      pvalue <- adonis2(dist(perm.eg.betadisper$distances) ~ as.factor(metadata$`metadata[keep]`))[1,5] 
      
      output_3[a,1] = pvalue
      
      if (a ==12) {
        tsne <- Rtsne(reads_table, dims = 2, perplexity=8, verbose=TRUE, max_iter = 5000)
        pic <- tsne$Y
        ...
        ggsave('NB_day_0_module_11_tsne.pdf', width=3, height=3)
      }
    }
    ...
    ord <- dbrda(reads_table2 ~ ., data = metadata, distance = "bray",parallel = 8)
    pvalue <- anova(ord, by = 'margin')
    pvalue = pvalue[-nrow(pvalue),]
    pvalue$order = str_remove_all(row.names(pvalue),'.*_')
    pvalue$order = as.numeric(as.character(pvalue$order))
    pvalue <- pvalue[order(pvalue$order),]
    output_4[,1] <- pvalue$`Pr(>F)`
...
#      sc_sp[n,] = sc_sp[n,]/2
      ...
      pdf("dbRDA_3_NB_Day_0.pdf",width = 19, height = 19)
      plot(dbRDA,
      ...
      text(sc_sp + c(0.08, -0.08), # adjust text coordinates to avoid overlap with points 
...
#            sc_sp[n,] = sc_sp[n,]/2
      ...
      # Set up a blank plot with scaling, axes, and labels
      pdf("dbRDA_3_NB_Day_12.pdf",width = 19, height = 19)
      plot(dbRDA,
           scaling = 1, # set scaling type 
           type = "none", # this excludes the plotting of any points from the results
           frame = FALSE,
  ...
  # NR day 0
  {
    ...
    for (a in 1:(nrow(pvalue_all))) {
      ...
      if (a ==3) {
        ...
        ggsave('NR_day_0_module_2_tsne.pdf', width=3, height=3)
      }
    }
    colnames(pvalue_all) = c('Df','SumOfSqs','R2','F','P_value')
    ...
    dbRDA = capscale(reads_table2 ~ ., data = metadata, distance = "bray",parallel = 8)
    {
      ...
      n = which(row.names(sc_sp) %in% c('Lactobacillus_iners','Streptococcus_infantis',
                                        'BVAB1','Escherichia_coli','Bacteroides_fragilis'))
      sc_sp[n,] = sc_sp[n,]/2
      ...
      # Set up a blank plot with scaling, axes, and labels
      pdf("dbRDA_3_NR_Day_0.pdf",width = 9, height = 9)
      plot(dbRDA,
           scaling = 1, # set scaling type 
           type = "none", # this excludes the plotting of any points from the results
           frame = FALSE,
  ...
  # NR day 1 2
  {
    ...
    dbRDA = capscale(reads_table2 ~ ., data = metadata, distance = "bray",parallel = 8)
    {
      ...
      sc_bp = sc_bp*8
                  ...
                  n = which(row.names(sc_sp) %in% c('Escherichia_coli','Enterococcus_faecalis',
                                                    'Staphylococcus_epidermidi','Streptococcus_mitis',
                                                    'Lactobacillus_iners'))
                  sc_sp[n,] = sc_sp[n,]/2
      ...
      # Set up a blank plot with scaling, axes, and labels
      pdf("dbRDA_3_NR_Day_12.pdf",width = 19, height = 19)
      plot(dbRDA,
           scaling = 1, # set scaling type 
           type = "none", # this excludes the plotting of any points from the results
           frame = FALSE,
  ...
  # NS day 1 2
  {
    ...
    metadata = metadata[,c(2,3,4,5,6,7,8,9,1,10,11,12,13,14)]
    ord <- dbrda(reads_table2 ~ ., data = metadata, distance = "bray",parallel = 8)
    pvalue <- anova(ord, by = 'margin')
    pvalue = pvalue[-nrow(pvalue),]
    pvalue$order = str_remove_all(row.names(pvalue),'.*_')
    ...
    dbRDA = capscale(reads_table2 ~ ., data = metadata, distance = "bray",parallel = 8)
    {
      ...
      n = which(row.names(sc_sp) %in% c('Escherichia_coli','Staphylococcus_epidermidis',
                                        'Bacteroides_OTU_223',
      ...
      # Set up a blank plot with scaling, axes, and labels
      pdf("dbRDA_3_NS_Day_12.pdf",width = 9, height = 9)
      plot(dbRDA,
           scaling = 1, # set scaling type 
           type = "none", # this excludes the plotting of any points from the results
           frame = FALSE,
  ...
  save_heatmap_pdf(p, 'Beta_p_multiple_variables_marginal_test.pdf', width=3, height=3.5)
  
  x = output_3; x[x >0.05] = 1;
  p = pheatmap(as.matrix(x),cluster_rows = F,cluster_cols = F,color=myColor,
               display_numbers = as.matrix(output_3))
  save_heatmap_pdf(p, 'Beta_p_betadisper.pdf', width=3, height=3.5)
  
  x = output_4; x[x >0.05] = 1;
  p = pheatmap(as.matrix(x),cluster_rows = F,cluster_cols = F,color=myColor,
               display_numbers = as.matrix(output_4))
  save_heatmap_pdf(p, 'Beta_dbRDA_p.pdf', width=3, height=3.5)
