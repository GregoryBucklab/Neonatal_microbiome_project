...
# get blank control reads table
{
  ...
  data = read.table('16s.momspi.new_reads_blank.txt', header = T, sep = '\t')
  
  setwd('/Users/binzhu/Desktop/Mom-baby')
  data = data[str_detect(data$SampleID,'Alpha-Water'),]

  data$Threshold.Status[data$Threshold.Status >= 97] = 'AT'
  ...
  
  {
    ...
    reads_table_16s_blank = as.data.frame(matrix(data = 0, ncol = length(sample_list) , nrow = length(taxa_list)))
    colnames(reads_table_16s_blank) = sample_list
    row.names(reads_table_16s_blank) = taxa_list
    
    for (a in 1: length(data_3)) {
      data_4 = data_3[a]
      data_3[a] = NA
      data_4 = unlist(data_4)
      reads_table_16s_blank[,a] = as.numeric(as.character(data_4))
    }
  }
  ...
  keep = rowSums(reads_table_16s_blank) != 0;sum(keep)
  reads_table_16s_blank = reads_table_16s_blank[keep,]
  
  row.names(reads_table_16s_blank) = str_replace_all(row.names(reads_table_16s_blank),'GG','gg')
  data = as.data.frame(row.names(reads_table_16s_blank))
  colnames(data) = 'rowname_16s'
  
  data$Taxa_name = NA
  data$Taxonomy = NA
  data$reads = rowSums(reads_table_16s_blank)
  
  keep = taxonomy$V1 %in% data$rowname_16s
  sum(keep)
  data_2 = taxonomy[keep,]
  data_2 = data_2[match(data$rowname_16s,data_2$V1),]
  
  taxa_list = unique(data_2$V3)
  reads_table_16s_blank$taxa = data_2$V3
  
  data = as.data.frame(table(reads_table_16s_blank$taxa))
  reads_table_16s_blank_1 = reads_table_16s_blank[reads_table_16s_blank$taxa %in% data$Var1[data$Freq ==1],]
  reads_table_16s_blank_2 = reads_table_16s_blank[reads_table_16s_blank$taxa %in% data$Var1[data$Freq >1],]
  
  {
    data_list <- split(reads_table_16s_blank_2, f = reads_table_16s_blank_2$taxa)
    ...
    reads_table_16s_blank_2$taxa = NULL
    for (a in 1:length(taxa_list)) {
      reads_table_16s_blank_2[a,] = data[[a]]
    }
    reads_table_16s_blank_2 = reads_table_16s_blank_2[c(1:length(taxa_list)),]
    row.names(reads_table_16s_blank_2) = taxa_list
    
    row.names(reads_table_16s_blank_1) = reads_table_16s_blank_1$taxa
    reads_table_16s_blank_1$taxa = NULL
    
    reads_table_16s_blank = rbind(reads_table_16s_blank_1,reads_table_16s_blank_2)
  }
  
  row.names(reads_table_16s_blank) = str_replace_all(row.names(reads_table_16s_blank), '_clade','_OTU')
...
{
  # total reads
  data = as.data.frame(colSums(reads_table_16s_blank))
  colnames(data) = 'Sample total reads'

  data$`Sample total reads`[data$`Sample total reads` > 5000] =5000

  ggplot(data, aes(x=`Sample total reads`)) + 
    geom_density()+theme_bw()
  ggsave('blank_control_total_sample_reads.pdf',width = 3, height = 2.5)
  
  
  data = as.data.frame(rowSums(reads_table_16s_blank))
  colnames(data) = 'Taxon total reads'
  data$Average_reads_per_sample = data$`Taxon total reads`/ncol(reads_table_16s_blank)
  write.csv(data, 'blank_control_Taxa_reads.csv')
  
  data$`Sample total reads`[data$`Taxon total reads` > 5000] =5000
  
  ggplot(data, aes(x=`Taxon total reads`)) + 
    geom_density()+theme_bw()
  ggsave('blank_control_total_Taxa_reads.pdf',width = 3, height = 2.5)
  
  colnames(reads_table_16s_blank) = paste0('Pseudo_ID_',c(1:ncol(reads_table_16s_blank)))
  write.csv(reads_table_16s_blank, 'reads_table_16s_blank.csv')
}
