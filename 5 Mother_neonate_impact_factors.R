...
for (a in 1: length(type1_all)) {
  ...
  # worries
  { if ( a==1) {
    n = which(str_detect(colnames(metadata), 'on_top_things_worries|control_worries|things_goingaway_worries|able_control_irritation_worries|money_worries|something_unexpected_worries|
                         health_worries|difficulties_pileup_worries|notcope_things_worries'))
    
    #    n = which(str_detect(colnames(metadata), 'worries|mental|depression|anxiety|mood|stress'))
    #    n = which(str_detect(colnames(metadata), 'worries'))
    #    metadata_2 = as.matrix(metadata[,n])
    
    m = which(str_detect(colnames(metadata_2), 'personal_problems_worries|on_top_things_worries|
                         able_control_irritation_worries|things_goingaway_worries'))
    metadata_3 = metadata_2[,-m]
    metadata_4 = metadata_2[,m]
    metadata_4 = metadata_4+10
    metadata_4[metadata_4 ==10] = 4
    metadata_4[metadata_4 ==11] = 3
    metadata_4[metadata_4 ==12] = 2
    metadata_4[metadata_4 ==13] = 1
    metadata_4[metadata_4 ==14] = 0
    metadata_2 = cbind(metadata_3,metadata_4)
    metadata_2[,colnames(metadata_2) == 'money_worries'] = metadata_2[,colnames(metadata_2) == 'money_worries'] -1
    
    #    metadata_3 = rowMedians(metadata_2, na.rm = T)
    
    metadata_3 = metadata_2 >=3
    metadata_3[is.na(metadata_3)] = F
    metadata_3 = rowSums(metadata_3)
    metadata_3[metadata_3>3] = 3
    
    
    temp = as.data.frame(cbind(shannon_2,metadata_3))
    colnames(temp) = c('V1','V2')
    temp$V1 = as.numeric(as.character(temp$V1))
    temp$V2 = (as.factor(temp$V2))
    kruskal.test(temp$V1, temp$V2)$p.value
    ...
    ggsave(paste0('alpha_shannon_',type1_all[a],'.pdf'),width=3, height=3)
