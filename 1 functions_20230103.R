...
get_abundance_table <- function(reads_table, mc.cores = 1) {
...
dif_abundance_2 <- function(reads_table,metadata, pvalue_th = 0.05, fold_change_th = 1, paired_test = F, order_reverse = F, style = 1, order = NA) {
...
newwork_rcorr <- function(reads_table, normalization_method = NA, type = 'spearman', pvalue = 0.05, cor_parameter= 0, style = 1, bar_max = 2, bar_min = -2, pheatmap_fontsize = 5, treeheight = 50, alpha = 0.05) {
  ...
  if (is.na(normalization_method)) {
    ...
    reads_table <- as.matrix(clr(reads_table))     ### CLR normalization in rows
