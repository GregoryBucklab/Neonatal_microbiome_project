...
prepare_reads_table_2 = function(reads_table, total_reads_threshold = 5000, species_threshold = 0.001, mc.cores = 1) {
  ...
  return(reads_table = reads_table)
}
...
get_abundance_table <- function(reads_table, mc.cores = 1) {
  ...
  reads_table_abundance_2
  
  return(reads_table_abundance_2)
}
...
beta_diversity = function(reads_table, metadata = NA, factor_name = NA, order = NA, NMDS_skip = T, ref_group = NA, rarefy_to = NA, pheatmap_fontsize = 5,treeheight = 50, pheatmap_y = T) {
 ...
 #     distance_median[lower.tri(distance_median)] <- NA
      ...
      save_pdf(group_dis_2_p, 'pheatmap.pdf')
    }
...
######### diffential abundance2 ### dif_abundance; sample in column; paired text or not; change order of species; fold change threshold; if style = 1, give dot plot, else give box plot ####################
dif_abundance2 <- function(reads_table,metadata,paired_test = F, order_reverse = F, fold_change_th = 1, style = 1, order = NA, alpha = 0.05) {
  
  reads_table=cts3
  metadata=as.character(metadata3$Delivery)
  style =2
  order = c('TB','PTB')
  fold_change_th = 1
  order_reverse = F
  ...
  das = as.data.frame(matrix(data = NA, nrow = nrow(reads_table), ncol = 6))
  colnames(das) = c('Species','Median abundance change','Mean abundance change','-Log10(adj-pvalue)','-Log10(pvalue)','Host')
  for (a in 1: nrow(reads_table)) {
    keep = metadata == order[1]
    
    factor1 = as.numeric(as.character(reads_table[a,keep]))
    factor2 = as.numeric(as.character(reads_table[a,!keep]))
    
    pvalue = wilcox.test(factor1, factor2, paired = paired_test)$p.value
    
    das[a,5] = pvalue
    das[a,2] = median(factor2) / median(factor1)
    das[a,3] = mean(factor2) / mean(factor1)
    das[a,1] = row.names(reads_table)[a]
    das[a,6] = host_list_e2[a,3]
  }
  
  das$`-Log10(pvalue)` = as.numeric(as.character(das$`-Log10(pvalue)`))

  adj_r <- adjust.p(das$`-Log10(pvalue)`, pi0.method="bky", alpha = alpha,pz = 0.05)  # set alpha for fdr. Another function - p.adjust - use alpha = 0.25
  adj_r <- adj_r$adjp
  das$`-Log10(adj-pvalue)` <- adj_r$adjusted.p
  
  das$`-Log10(pvalue)` = -log10(das$`-Log10(pvalue)`)
  das$`-Log10(adj-pvalue)` = -log10(das$`-Log10(adj-pvalue)`) 
    
  das$abundance_change_das = log2(das$`Mean abundance change`)
  das$abundance_change_das[das$abundance_change_das == Inf] = 100
  
  # draw figure
  das <- das[(das$`-Log10(pvalue)` >=1.301 & (das$abundance_change_das >=fold_change_th | das$abundance_change_das <=-fold_change_th)),]
...
######### diffential abundance3 ### no plot, output csv no matter change or not ####################
dif_abundance3 <- function(reads_table,metadata) {
