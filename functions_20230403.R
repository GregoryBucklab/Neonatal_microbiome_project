...
beta_diversity = function(reads_table, metadata = NA, factor_name = NA, 
  ...
  within_dis_p = ggplot(within_dis, aes(x=Source, y=value)) + geom_violin(trim=T)+
    geom_boxplot(fill='white', color="black", outlier.shape=NA, width=0.1) +
    geom_jitter(size = 0.1)+ theme_bw()+
...
#   pvalue_th = 0.05
  ...
  if (style == 1) {
    ...
    p <- ggplot(reads_table_3, aes(x=Taxa, y=Abundance,fill=Type)) + geom_violin(trim=T)+
      geom_boxplot(fill='white', color="black", outlier.shape=NA, width=0.1) +
      geom_jitter(size = 0.1)+theme_bw()+
      ylab("Abundance (%)")+
...
barplot <- function(reads_table, type_th = 0.1, taxa_num = 9, VMB = T) { # pass proportion data, samples on rows, taxa on columns.
  ...
  if (VMB == F) {
    ...
    p <- ggplot(ggplotdata, aes(SampleID, ATprop, fill=mycolors, group=ATprop)) + 
      geom_bar(stat="identity", position="stack", width=1.5, binwidth=0) +
    ...
    # set color
    {
      ...
      mycolors[grep("Bifidobacteriaceae",        colnames(mypropdata))]     <- "#00A2A7"
      mycolors[grep("Bradyrhizobium",        colnames(mypropdata))]     <- "#CDA400"
      mycolors[grep("Mycoplasma",        colnames(mypropdata))]     <- "#9467bd"
...
######### network for numeric data ############# sample in cols #######
newwork_rcorr <- function(reads_table, normalization_method = NA, type = 'spearman', pvalue = 0.05, cor_parameter= 0, 
  ...
  return(c(list(p = p), list(gephi_input = gephi_p.yes.rr),list(pvalue = pvalue_output),
           list(adj_pvalue = otu.pval),list(Rvalue = r.val_output), list(cor_matrix = p.yes.rr)))
}
...
######### network for categorical data ############# sample in cols #######
newwork_chi_squared <- function(reads_table, pvalue = 0.05, style = 1,
                                pheatmap_fontsize = 5, treeheight = 50, alpha = 0.05,FDR = T) {
  reads_table = t(metadata)
  reads_table = as.data.frame(t(reads_table))

  output = data.frame(matrix(data = NA, nrow = 0, ncol = 5))
  colnames(output) = c('Variable_1','Variable_2', 'X_squared', 'P_value', 'Adj_P_value')
  for (a in 1:(ncol(reads_table)-1)) {
    for (b in (a+1):ncol(reads_table)) {
      output_2 = data.frame(Variable_1 = colnames(reads_table)[a], 
                            Variable_2 = colnames(reads_table)[b],
                            X_squared = NA, P_value = NA, Adj_P_value = NA)
      data = data.frame(V1 = reads_table[,a], V2 = reads_table[,b])
      data = data[!is.na(data$V1) & !is.na(data$V2),]
      x = chisq.test(table(data$V1,data$V2))
      
      output_2$X_squared = as.numeric(as.character(x$statistic))
      output_2$P_value = as.numeric(as.character(x$p.value))
  ...
  if (FDR == T) {
    x <- adjust.p(output$P_value, pi0.method="bky", alpha = alpha,pz = 0.05)  # set alpha for fdr. Another function - p.adjust - use alpha = 0.25
    x <- x$adjp
    output$Adj_P_value <- x$adjusted.p
  }
  
  # gephi output
  gephi_p.yes.rr = output
  gephi_p.yes.rr = gephi_p.yes.rr[gephi_p.yes.rr$Adj_P_value <= pvalue,]
  gephi_p.yes.rr = gephi_p.yes.rr[,c(1,3,2)]
  colnames(gephi_p.yes.rr) = c('Source','Weight','Target')
  
  # get p.yes.rr matrix
  variable_list = unique(c(gephi_p.yes.rr$Source,gephi_p.yes.rr$Target))
  p.yes.rr = matrix(data = 0, ncol=length(variable_list), nrow=length(variable_list))
  colnames(p.yes.rr) = variable_list; row.names(p.yes.rr) = variable_list
  
  for (a in 1:nrow(gephi_p.yes.rr)) {
    x = which(colnames(p.yes.rr) == gephi_p.yes.rr$Source[a])
    y = which(colnames(p.yes.rr) == gephi_p.yes.rr$Target[a])
    p.yes.rr[x,y] = gephi_p.yes.rr$Weight[a]; p.yes.rr[y,x] = gephi_p.yes.rr$Weight[a]; 
  }
  ...
  detach("package:igraph", unload = TRUE, force = T)
  return(c(list(p = p), list(gephi_input = gephi_p.yes.rr),list(pvalue = output), list(cor_matrix = p.yes.rr)))
}


######### newwork_rcorr2 correlation of two paired normalized reads table ############# sample in col #######
newwork_rcorr2 <- function(reads_table1, reads_table2, type = 'spearman', pvalue = 0.05, cor_parameter= 0, style = 1, bar_max = 2, 
                           bar_min = -2, pheatmap_fontsize = 5 , treeheight = 50, alpha = 0.05, mc.cores =1, pvalue_adj = T) {
  #    cor_parameter= 0
  #    pvalue = 0.05
  #    style = 1
  #    bar_max = 2
...
diversity_analysis <- function(reads_table_ori,metadata_ori, key_words) {
  {
    data = prepare_reads_table(reads_table_ori, metadata_ori, total_reads_threshold = 5000, 
                               species_threshold = 0.001, mc.cores = 8)
    reads_table_ori = data$reads_table
    metadata_ori = data$metadata
    metadata = metadata_ori[,str_detect(colnames(metadata_ori), key_words)]
  }
  
  {
    ...
    ggplot(data=alpha, aes(x=Group, y=Shannon)) +geom_violin(trim=T)+
      geom_boxplot(fill='white', color="black", outlier.shape=NA, width=0.1) +
      geom_jitter(size = 0.1)+theme_bw()+
      theme_classic()+
      theme(axis.title = element_text(size = 7), 
    ...
    ggplot(data=alpha, aes(x=Group, y=Evenness)) +geom_violin(trim=T)+
      geom_boxplot(fill='white', color="black", outlier.shape=NA, width=0.1) +
      geom_jitter(size = 0.1)+theme_bw()+
      theme_classic()+
      theme(axis.title = element_text(size = 7), 
    ...
    ggplot(data=alpha, aes(x=Group, y=Ovserved_taxa)) +geom_violin(trim=T)+
      geom_boxplot(fill='white', color="black", outlier.shape=NA, width=0.1) +
      geom_jitter(size = 0.1)+theme_bw()+
      theme_classic()+
      theme(axis.title = element_text(size = 7), 
    ...
    # beta diversity
    beta = beta_diversity(reads_table_ori, metadata = metadata, factor_name = key_words, order = NA, NMDS_skip = F, ref_group = NA, rarefy_to = NA, pheatmap_fontsize = 5,treeheight = 50)
    beta$within_dis_p+
      theme_classic()
    ...
    tsne <- Rtsne(reads_table, dims = 2, perplexity=20, verbose=TRUE, max_iter = 5000)
    pic <- tsne$Y
    metadata = metadata_ori[,str_detect(colnames(metadata_ori), key_words)]
    pic <- data.frame(pic,metadata)
...
FilterFeatures <- function(input, output, p.value_th = 0.05)      # remove columns with Standard deviation == 0 
{
  # remove sds = 0
  library(matrixStats)
  ...
  keep = sapply(1:ncol(input), function(j) ( cor.test(input[,j], as.numeric(as.factor(output)), method = 'spearman')$p.value <=p.value_th))
...
xxx_get_result <- function(input, output, ntree = 500) {
  ...
  xxx<-function(X, Ccohort, i, ntree=ntree) # X is the input data; Ccohort is the output data; i is seq(length(alpha.shannon_diversity))
  {
    library(randomForest)
    library(forestError)
    set.seed(2018+123*i)
...
######### sample size power on adonis test;  reads_table sample in rows; metadata is a vector #####
# https://medium.com/brown-compbiocore/power-analyses-for-microbiome-studies-with-micropower-8ff28b36dfe3
# https://gist.github.com/aguang/774862ab8ef2c0b383666ab9791aa2cf#effect
adonis_power <- function(reads_table, metadata) {
  library(vegan)
  library(parallel)
  library(micropower)
  metadata = as.character(metadata)
  
  Bray_curtis<-as.matrix(vegdist(reads_table, method = "bray", binary=FALSE)) # compute the pairwise unweighted Bray_curtis distances
  m<-mean(Bray_curtis) # the mean and standard deviation of the matrix to get the necessary population parameters
  s<-sd(Bray_curtis) 
  
  cores = detectCores()
  
  unique_metadata_list = unique(metadata)
  n = sum(metadata == unique_metadata_list[1])
  n2 = sum(metadata == unique_metadata_list[2])
  
  # Determine rarefaction and number of OTUs
  weight_hm<-hashMean(rare_levels=runif(100,0,1),rep_per_level=10,otu_number=ncol(reads_table),sequence_depth=5000) # use hashMean to simulate a list of OTU tables at different levels of subsampling (rare_levels) so that we can determine what level of subsampling produces OTU tables with the same mean pairwise distance.
  # 1000 is number of simulated samples 
  ...
  weight_hsd<-hashSD(rare_depth=subsample[1],otu_number_range=2^runif(n = 10,min = 5,max = 7),sim_number= n+n2, sequence_depth=5000)
  ...
  sp<-simPower(group_size_vector=c(n,n2), otu_number=otunum, rare_depth=subsample[1],sequence_depth=5000,effect_range=seq(0,0.3,length.out=100))
  ...
  bp30<-bootPower(wj, boot_number=10, subject_group_vector=c(n,n2),alpha=0.05)
  ...
  bp30_lm <- lm(log_power ~ log_omega2, data=bp30_model)
  return(bp30_lm)
}
#  effect_size = 10^predict(power_size_model_lm, newdata=data.frame(log_power=log10(effect_size)))
