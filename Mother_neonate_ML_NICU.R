...
for (sample_type in c("MV1D","MRCD","MCKD")) {
  ...
  { 
    ...
    metadata_mom_all = metadata_16s[keep,]
    
    keep = !is.na(metadata_mom_all$days_rel2birth)
    metadata_mom_all = metadata_mom_all[keep,]
    
    keep = !is.na(metadata_mom_all$baby_nicu)
    metadata_mom =metadata_mom_all[keep,]  # remove data without icu.    156 baby in total with icu information.
    
    keep = metadata_mom$SampleType == sample_type
    metadata_mom =metadata_mom[keep,] 
    icu = metadata_mom$baby_nicu

    data = data.frame(icu=icu, week_pregnancy=metadata_mom$weeks_pregnant)
    week_unique = unique(data$week_pregnancy)
    data_2 = as.data.frame(matrix(data = NA, nrow = length(week_unique)*2, ncol = 3))
    colnames(data_2) = c('week_pregnancy', 'NICU', 'Case_number')
    data_2$week_pregnancy = rep(week_unique,2)
    data_2$NICU = c(rep('Yes', length(week_unique)), rep('No', length(week_unique)))
    
    for (a in seq(length(week_unique))) {
      data_2$Case_number[a] = sum(data$icu == 'Yes' & data$week_pregnancy == week_unique[a])
      data_2$Case_number[a+length(week_unique)] = sum(data$icu == 'No' & data$week_pregnancy == week_unique[a])
    }
    data_2 = data_2[order(data_2$week_pregnancy),]
    ggplot(data_2, aes(week_pregnancy, Case_number, fill=NICU)) + 
      geom_bar(stat="identity", position="stack", width=1) 
    
    ggsave(paste0(sample_type,'_NICU_case_number.pdf'), width=8, height=3)
...
#####  metadata #####
# metadata modeling ist visit   prepare data
{
  ...
  keep = !is.na(metadata_mom_all$baby_nicu)
  metadata_mom =metadata_mom_all[keep,]  # remove data without icu.
  ...
  keep = colSums(is.na(metadata_mom)) <= 50
  metadata_mom = metadata_mom[,keep]
...
# modeling
{
  ...
  data$p_roc
  ggsave('icu_M_meta_Sensitivities_Specificities.pdf',width=3, height=3)
...
# prepare data
{
  ...
  keep = !is.na(metadata_mom_all$baby_nicu)
  metadata_mom_all =metadata_mom_all[keep,]  # remove data without icu.    156 baby in total with icu information.
  
  a=2
  keep = metadata_mom_all$weeks_pregnant >= a*5 + 1 & metadata_mom_all$weeks_pregnant < (a+1)*5 + 1
  metadata_mom_all =metadata_mom_all[keep,]
  ...
  icu = metadata_mom_all$baby_nicu
  ...
  metadata = FilterFeatures(metadata, icu)
...
# modeling
{
  ...
  data$p_roc
  ggsave('icu_M_meta_week6_11_Sensitivities_Specificities.pdf',width=3, height=3)
...
# prepare data
{
  ...
  keep = metadata_mom_all$weeks_pregnant <= 21
  metadata_mom_all =metadata_mom_all[keep,]
...
# microbiomes at 1st half pregnancy
{
  setwd('/Users/binzhu/Desktop/Mom-baby/')
  colnames(metadata_16s)[colnames(metadata_16s) == 'mom_delivery_method (C-section yes / vaginal no)'] = 'mom_delivery_method'
...
for (sample_type in c("MV1D","MRCD","MCKD")) {
    ...
    {
      keep = metadata_mom_all$weeks_pregnant <= 21
      metadata = metadata_mom_all[keep,]
      
      keep = metadata$SampleType == sample_type
      metadata = metadata[keep,]
      
      Participant_list = metadata$ParticipantID
...
##### combined model 6-11 weeks #####
{
  # prepare data
  {
    ...
    metadata_icu_MV = metadata_mom_all[keep,]
    metadata_icu_MV = metadata_icu_MV[order(metadata_icu_MV$VisitNum, decreasing = F),]
    metadata_icu_MV = metadata_icu_MV[order(metadata_icu_MV$ParticipantID),]
    keep = !duplicated(metadata_icu_MV$ParticipantID)
    metadata_icu_MV = metadata_icu_MV[keep,]
    
    keep = metadata_mom_all$SampleType == "MRCD"
    metadata_icu_MR = metadata_mom_all[keep,]
    metadata_icu_MR = metadata_icu_MR[order(metadata_icu_MR$VisitNum, decreasing = F),]
    metadata_icu_MR = metadata_icu_MR[order(metadata_icu_MR$ParticipantID),]
    keep = !duplicated(metadata_icu_MR$ParticipantID)
    metadata_icu_MR = metadata_icu_MR[keep,] 
    
    keep = metadata_mom_all$SampleType == "MCKD"
    metadata_icu_MB = metadata_mom_all[keep,]
    metadata_icu_MB = metadata_icu_MB[order(metadata_icu_MB$VisitNum, decreasing = F),]
    metadata_icu_MB = metadata_icu_MB[order(metadata_icu_MB$ParticipantID),]
    keep = !duplicated(metadata_icu_MB$ParticipantID)
    metadata_icu_MB = metadata_icu_MB[keep,] 
    
    # metadata 
    keep = metadata_mom_all$ParticipantID
    keep_list =   keep[keep %in% metadata_icu_MV$ParticipantID &
                         keep %in% metadata_icu_MR$ParticipantID]
    
    keep = metadata_icu_MV$ParticipantID %in% keep_list
    metadata_icu_MV = metadata_icu_MV[keep,]
    metadata_icu_MV = metadata_icu_MV[order(metadata_icu_MV$ParticipantID),]
    
    keep = metadata_icu_MR$ParticipantID %in% keep_list
    metadata_icu_MR = metadata_icu_MR[keep,]
    metadata_icu_MR = metadata_icu_MR[order(metadata_icu_MR$ParticipantID),]
    ...
    get_data_for_nicu_1st <- function(metadata, reads_table) {
      ...
      reads_table_output = prepare_reads_table_2(reads_table_output, total_reads_threshold = 5000, species_threshold = 0.001, mc.cores = 8)
      keep = metadata$SampleID %in% colnames(reads_table_output)
      metadata = metadata[keep,]
      icu_output = metadata$baby_nicu
      ...
      return(c(reads_table_output = list(reads_table_output), 
               icu_output = list(icu_output), 
               metadata = list(metadata)))
    }
    
    data = get_data_for_nicu_1st(metadata_icu_MV, reads_table_16s)
    reads_table_16s_MV = data$reads_table_output
    metadata_icu_MV = data$metadata
    
    data = get_data_for_nicu_1st(metadata_icu_MR, reads_table_16s)
    reads_table_16s_MR = data$reads_table_output
    metadata_icu_MR = data$metadata
    
    icu_2 = metadata_mom_all$baby_nicu
    ...
    metadata = FilterFeatures(metadata, icu_2)
    metadata = metadata[,!str_detect(colnames(metadata), 'average|baby|ga_at_delivery|event
                                     |pregnancy|last_weight|last_pulse
                                     |last_ga_at_delivery|mom_feeding_method|last_weight
                                     |hispanic_or_latino|vaginal_penetration_frequency
  ...
  # calculate weight
  {
    ...
    for(i in seq(nrow(metadata)) )
    {
      ...
      prf[1]=wilcox.test(ppp_M_meta_combine , as.numeric(as.factor(icu_2[-i])))$p.value
      prf[2]=wilcox.test(ppp_MV_1st_combine , as.numeric(as.factor(icu_2[-i])))$p.value
      prf[3]=wilcox.test(ppp_MR_1st_combine , as.numeric(as.factor(icu_2[-i])))$p.value
    ...
    myPvs[n_ML]=roc(icu_2, CMBW)$auc
    myPvs2[n_ML]=-log10(wilcox.test(CMBW ~ as.factor(icu_2))$p.value)
    myPvs4[n_ML] = paste0(length(icu_2),' (',sum(icu_2 == 'Yes'),')')
  }
...
##### visualize results #####
{
  ...
  data = data.frame(Real_value=icu_2, Predicted_value= CMBW)
  ...
  model_name =c('MV 11-15 weeks','MV 16-20 weeks','MV 21-25 weeks',
                ...
                'MV <= 21 weeks','MR <= 21 weeks','MB <= 21 weeks','Integrated model 11-15 weeks')
  ...
  ggplot(data, aes(Model_name, p_value)) + 
    geom_bar(stat="identity", width = 0.7,color = 'black', fill = 'gray') + scale_y_continuous(breaks = seq(0, 3, 0.5)) +
...
##### combined model <=21 weeks not good #####
{
  # prepare data
  {
    setwd('/Users/binzhu/Desktop/Mom-baby/')
  ...
  # calculate weight
  {
    ...
    for(i in seq(nrow(metadata)) )
    {
      ...
      prf[1]=wilcox.test(ppp_M_meta_combine , as.numeric(as.factor(icu_2[-i])))$p.value
      #     prf[2]=wilcox.test(ppp_MV_1st_combine , as.numeric(as.factor(icu_2[-i])))$p.value
      prf[2]=wilcox.test(ppp_MR_1st_combine , as.numeric(as.factor(icu_2[-i])))$p.value
      
      prf=exp(-log10(prf))
      prf_all[[i]] = prf
      prf_all_sum[i] = sum(prf)
      CMBW[i]=(prf[1]*ppp_metadata_1st[i]+prf[2]*ppp_MR_1st[i])/sum(prf)   # first layer expected value * -log(second layer pvalue) / sum(-log(second layer pvalue))
      #      CMBW[i]=(prf[1]*ppp_metadata_1st[i]+prf[2]*ppp_MV_1st[i]+prf[3]*ppp_MR_1st[i])/sum(prf)   # first layer expected value * -log(second layer pvalue) / sum(-log(second layer pvalue))
    }
    roc(icu_2, CMBW)$auc
