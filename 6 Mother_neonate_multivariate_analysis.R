...
# beta adonis
{
  ...
  write.csv(metadata_2_imputation,'mom_baby_metadata_2_imputation.csv')
  ...
  
  {
    ...
    pvalue_all = as.data.frame(matrix(data = NA, ncol = 2, nrow = ncol(metadata_2_imputation)))
    ...
    func_1 = function(trial) {
      ...
      pvalue <- c(pvalue[1,3],pvalue[1,5])
      return(pvalue)
    }
    ...
    for (a in 1:(nrow(pvalue_all))) {
      pvalue_all$V1[a] = pvalue[a*2-1]
      pvalue_all$V2[a] = pvalue[a*2]
    }
    colnames(pvalue_all) = c('R2','P-value')
    ...
    metadata = as.data.frame(metadata_2_imputation[,c(1:138)])
...
# beta adonis
{
  ...
  
  {
    ...
    for (a in 1:(nrow(pvalue_all))) {
      ...
      pvalue <- adonis2(reads_table ~ ., data = metadata, 
                        method = "bray", parallel = 60)
      pvalue_all$V1[a] = pvalue[1,3]
      pvalue_all$V2[a] = pvalue[1,5]
    }
    ...
    pvalue_all = pvalue_all[order(pvalue_all$V2, decreasing = F),]
    metadata_2_imputation = metadata_2_imputation[row.names(pvalue_all)]
    
    metadata = as.data.frame(metadata_2_imputation[,c(1:142)])
    pvalue <- adonis2(reads_table ~ ., data = metadata,method = "bray", parallel = 60)
    pvalue
  ...
  {
    {
      ...
      
      {
        ...
        metadata = as.data.frame(metadata_2_imputation[,c(1:83)])
        pvalue <- adonis2(reads_table ~ ., data = metadata,method = "bray", parallel = 60)
        pvalue
  ...
  {
    {
      ...
      
      {
        ...
        metadata = as.data.frame(metadata_2_imputation[,c(1:53)])
        pvalue <- adonis2(reads_table ~ ., data = metadata,method = "bray", parallel = 60)
        pvalue
